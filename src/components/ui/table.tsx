...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-9 px-3 text-left align-middle text-[12px] font-semibold text-muted-foreground uppercase tracking-wider",
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "px-3 py-2.5 align-middle text-[14px] [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
